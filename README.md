...
4. Для каждого сообщения:
   ...
   - В фоне запускается заливка файла в Google Drive (сохраняется та же структура папок)
5. Пользователь видит в чате расшифровку и подтверждение сохранения

Локальный диск — canonical source of truth, Drive — резервная копия.
...
```
.
├── bot.py              # точка входа, регистрация хендлеров, error handler
├── handlers.py         # хендлеры диалога регистрации и приёма медиа
├── states.py           # константы состояний ConversationHandler
├── user_manager.py     # сохранение профиля и feedback.jsonl, триггер Drive
...
- Папка `users/` с персональными данными также не коммитится
